...
# Create a list with the columns that won't be used
input_features = ['TRANSACTION_ID', 'TX_DATETIME', 'CUSTOMER_ID', 'TERMINAL_ID', 'TX_TIME_SECONDS', 'TX_TIME_DAYS', 
'TX_FRAUD_SCENARIO']

df = df_train_full.drop(input_features, axis=1)
...
# -------------------------------------- SUBSAMPLING THE DATAFRAME APPROACH --------------------------------------------
# Undersampling data
...
# Getting the number of features in the subsampled data
n_sub_inputs = X_train.shape[1]

# Defining a neural structure to the deep learning model of the subsampled data
...
# Input the confusions matrix with the results of classes predicts of subsampled neural model
sub_cm = confusion_matrix(original_ytest, sub_fraud_preds)

# ------------------------------------- OVERSAMPLING THE DATAFRAME APPROACH --------------------------------------------
# First model will be a logistic regression
...
for train, test in sss.split(original_Xtrain, original_ytrain):
    # Oversampling happens during Cross Validation, not before, to avoid data leakage
    pipeline = imbalanced_make_pipeline(SMOTE(sampling_strategy='minority'),
                                        rand_log_reg)
    model = pipeline.fit(original_Xtrain[train], original_ytrain[train])
    best_est = rand_log_reg.best_estimator_

# Get logistic regression predicts of oversampled data
...
# Second model will be the deep learning model of oversampled data
# Now to train a deep learning model, I've implemented the SMOTE technique before cross-validation
sm = SMOTE(sampling_strategy='minority', random_state=42)

# This will be the data we will fit to check oversample results in the deep learning model
Xsm_train, ysm_train = sm.fit_resample(original_Xtrain, original_ytrain)

# Deep learning model to ovesample
